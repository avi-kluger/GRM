...
#' @import psych
#' @import ggmirt
#' @importFrom ggplot2 ggsave labs theme_minimal theme geom_vline geom_line ggplot aes
#' 
#' @export
#' 
...
create_best_items_plot <- function(item_analysis, item_labels) {
  ...
  plot_data <- item_analysis
  plot_data$Item_Label <- paste0(plot_data$Item, ": ", substr(plot_data$Label, 1, 30), "...")
  
  # Create discrimination quality categories
  plot_data$Quality <- ifelse(plot_data$Discrimination >= 1.5, "Excellent (a≥1.5)",
                       ifelse(plot_data$Discrimination >= 1.0, "Good (1.0≤a<1.5)", "Poor (a<1.0)"))
  
  # Sort by discrimination for better visualization
  plot_data <- plot_data[order(plot_data$Discrimination, decreasing = TRUE), ]
  plot_data$Item_Label <- factor(plot_data$Item_Label, levels = plot_data$Item_Label)
  
  # Create the plot
  ggplot(plot_data, aes(x = Item_Label, y = Discrimination, fill = Quality)) +
    geom_col() +
    scale_fill_manual(values = c("Excellent (a≥1.5)" = "#2E8B57", 
                                 "Good (1.0≤a<1.5)" = "#4682B4", 
                                 "Poor (a<1.0)" = "#DC143C")) +
    labs(title = "Item Discrimination Analysis",
         subtitle = "Items ranked by discrimination parameter (a)",
         x = "Items",
         y = "Discrimination (a)",
         fill = "Quality") +
    theme_minimal() +
    ...
    geom_hline(yintercept = c(1.0, 1.5), linetype = "dashed", alpha = 0.6)
}
...
run_grm <- function(data, 
  ...
  if (n_factors == 1) {
    ...
    if (exists('item_analysis') && nrow(item_analysis) > 0) {
      tryCatch({
        # Create best items visualization
        ...
        if (display_plots) print(p5)  # This will show in viewer
